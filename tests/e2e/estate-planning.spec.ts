...
let retainedSnapshotUrl = "";
let retainedSnapshotId = "";

test.describe.serial("estate planning", () => {
  test("allocates assets, prints a private summary, and exports the plan", async ({
    page,
  }) => {
    ...
    await page.getByLabel("Confirm password").fill("estate-e2e-owner-password");
    ...
    await page.getByRole("button", { name: "Create account" }).click();
    await expect(
      page.getByRole("heading", { name: "Family land" }),
    ).toBeVisible();

    await page.getByRole("link", { name: "Estate plan" }).click();
    await expect(
      page.getByRole("heading", { name: "Estate distribution" }),
    ).toBeVisible();
    ...
    await assetCard
      .getByRole("button", { name: "Save asset directive" })
      .click();
    ...
    await assetCard
      ...
      .selectOption({ label: "Amina Example" });
    ...
    await page
      .getByLabel("Jurisdiction or residence")
      .fill("Example jurisdiction");
    ...
    retainedSnapshotUrl = snapshotUrl;
    ...
    await expect(
      page.getByText("Obtain a current independent valuation."),
    ).toHaveCount(0);
  ...
  test("denies another user direct snapshot and aggregate access", async ({
    page,
  }) => {
    await page.goto("/signup");
    ...
    await page.getByLabel("Confirm password").fill("estate-e2e-other-password");
    await page.getByRole("button", { name: "Create account" }).click();
    await expect(page.getByRole("heading", { name: "Overview" })).toBeVisible();

    await page.goto(retainedSnapshotUrl);
    await expect(
      page.getByRole("heading", { name: "Page not found" }),
    ).toBeVisible();
    const deniedDownload = await page.request.get(
      `/api/estate/snapshots/${retainedSnapshotId}`,
    );
    ...
    await expect(
      page.getByRole("heading", { name: "Add beneficiary" }),
    ).toBeVisible();
  });
});

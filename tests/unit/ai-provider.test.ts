...
describe.sequential("AI provider persistence and limits", () => {
  ...
  test("accepts a 100,000,000 monthly token limit and rejects values beyond its bounds", async () => {
    ...
    expect(saveAiProviderSettings(aliceId, input)?.monthlyTokenLimit).toBe(
      100_000_000,
    );
    ...
    for (const monthlyTokenLimit of [100_000_001, 9_999, 100_000_000.5]) {
      ...
      expect(aiProviderSettingsInputSchema.safeParse(invalid).success).toBe(
        false,
      );
    ...
    expect(
      aiProviderSettingsInputSchema.safeParse(excessiveOutput).success,
    ).toBe(false);
    expect(() => saveAiProviderSettings(aliceId, excessiveOutput)).toThrow(
      "cannot exceed the monthly token limit",
    );

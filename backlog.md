...
- **Implementation:** [lib/services/account-history-import.ts](lib/services/account-history-import.ts) owns strict CSV/JSON parsing, row classification, replay projection, and atomic accepted-subset commit. Account-scoped preview and commit routes verify the session, trusted origin, ownership, file limits, and SHA-256 confirmation. The account import page publishes templates, shows privacy-aware target-account balances and paginated outcomes, and downloads CSV/JSON reports. Transaction external IDs are protected by an owner/account unique index, exported in CSV and portability v5, and legacy versions restore with null IDs. The global Settings importer and `/api/import/transactions` were removed.

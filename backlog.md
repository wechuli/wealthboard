...
- **Priority:** P1
- **Estimated effort:** Large
- **Current gap:** The Settings CSV importer resolves accounts from file fields, writes immediately after fail-fast validation, and assigns a new UUID idempotency key to every row. It cannot import JSON, preview projected balance changes, report mixed row outcomes, or prevent an identical account history from being imported again.
- **Evidence from the repository:** `importTransactionsCsv()` in [lib/services/portability.ts](lib/services/portability.ts) accepts account IDs or names, validates up to 10,000 rows, inserts the complete batch, and recalculates affected accounts. [components/settings-forms.tsx](components/settings-forms.tsx) exposes the global upload, while account detail pages have no import workflow. [db/schema.ts](db/schema.ts) has internal transaction UUIDs and request idempotency keys but no provider/source transaction ID.
...
- **Acceptance criteria:** CSV and JSON equivalents produce identical stored transactions; preview performs no writes; structural file failures perform no writes; valid rows can commit while invalid and duplicate rows remain excluded; re-importing the same file changes no balances; conflicting IDs never overwrite data; concurrent duplicate attempts are stopped by the database index; foreign and archived accounts are denied; accepted-row database failure rolls back all accepted rows; projected and final balances match replay exactly; transaction export includes external IDs; version 5 round-trips and versions 2 through 4 restore; no raw file or sensitive row is logged; deterministic tests cover sign, precision, date, replay/valuation ordering, 10,000 rows, duplicate races, rollback, and two-user denial; component and Playwright tests cover reports, confirmation, privacy mode, keyboard access, and 360/390/768/1024/1440 px layouts.

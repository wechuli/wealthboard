...
- **Portability:** Version 6 includes beneficiaries, plans, directives, allocations, and retained snapshots in the authenticated user's versioned JSON export and restore, with relationship validation and ID remapping. F17 version 7 retains every estate collection unchanged while adding position records. Older archives restore with an empty estate plan according to their converter. CSV account exports should not include beneficiary data, and a dedicated planning-document download must never include another user's records or authentication data.
...
- **Status:** Proposed on 2026-08-14.
- **Priority:** P1
- **Estimated effort:** Epic
- **Current gap:** Wealthboard flattens a stock, ETF, or fund into one monetary account balance. `accounts.currentValueMinor` stores the total, `purchase` and `sale` apply money-only effects, and an account valuation replaces the whole balance. The application cannot retain units, effective-dated unit prices, multiple instruments in one brokerage account, price freshness, or position-aware imports.
- **Evidence from the repository:** [db/schema.ts](db/schema.ts) has account money, monetary transactions, and absolute valuations but no instrument, quantity, trade, or price records. `transactionEffect()` in [lib/finance.ts](lib/finance.ts) treats `purchase` as an increase and `sale` as a decrease of a generic holding. [lib/services/account-history-import.ts](lib/services/account-history-import.ts) intentionally accepts only `external_id,type,amount,date,description,notes`, so a broker row cannot preserve quantity and price.
...
- **Cost-basis boundary:** An opening per-position cost basis may be stored as optional reference metadata, but the first release does not claim tax-grade basis or realized gains. Tax lots, disposal methods, reinvestment basis, splits, mergers, spin-offs, and jurisdiction-specific rules require a later explicit model and golden fixtures.
...
- **Portability:** Advance per-user JSON portability to version 7 with tracking mode, instruments, position events, linked cash relationships, security prices, and reconciliation observations while retaining every version 6 estate collection. Restore rejects owner fields, remaps all IDs, validates relationships, replays quantities and values, and rolls back on any negative position or inconsistent link. Versions 2 through 6 restore as balance accounts with empty position collections.
...
- **Delivery slices:** Phase 1 adds mode selection, instruments, opening holdings, manual/effective-dated prices, derived current/history values, freshness warnings, and strict holdings/price import. Phase 2 adds account cash, atomic buys/sells/dividends/fees, unified activity, corrections, and full investment-history import. Phase 3 adds reconciliation history, in-kind transfers, selected corporate actions, optional lot-aware performance, and an optional price provider only after provenance and failure behavior are proven.
...
- **Acceptance criteria:** An existing account remains byte-for-byte behaviorally equivalent under default balance mode, and crafted attempts to change mode after any financial event fail without mutation. A user can create an isolated position account, add multiple instruments, hold fractional quantities, update effective-dated prices, and record atomic buys/sells/cash effects without changing contribution totals. Current and historical account values reconcile exactly to replayed cash plus converted positions; future prices are never used; F4-configured missing/stale prices and rates remain explicit in account, goal, estate, dashboard, and report outputs. Oversells at the event date or caused later by backdated edits, same-date event-order violations, cross-user IDs, duplicate/conflicting imports, and partial investment commits fail without mutation. Holdings/trade/price import previews match final replay, re-import is idempotent, version 7 round-trips every relationship, versions 2 through 6 restore as balance mode, and focused unit/component/E2E tests cover precision, rounding, same-date ordering, fees, FX settlement, corrections, reconciliation, privacy, five responsive widths, and two-user isolation.
...
- **Priority:** P2
- **Estimated effort:** Medium
- **Issue:** User export now emits version 6 and restore converts versions 2 through 5 for transaction external IDs, goal records, institutions, and estate collections, but there is still no general converter registry or documented support window. F17 will require version 7 position collections without weakening older restore paths.
- **Improvement:** Document archive support windows, add version-dispatched parsers/converters, and keep exported calculation-independent source records forward portable. The version 7 converter sets `trackingMode = "balance"` and empty position collections for versions 2 through 6 rather than inferring units from money-only history.
- **Acceptance criteria:** The archive version changes only when the serialized contract changes; each supported version has an isolated parser and converter; unsupported versions return a clear message; at least one fixture per supported version validates conversion and round-trip; version 7 rebuilds and verifies position quantities and values; removals follow the documented support window.

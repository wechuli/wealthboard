  ...
  if (!visible || amount === null) {
    return (
      <span className="text-slate-500">
        {amount === null ? "Rate unavailable" : "Value excluded"}
      </span>
    );
  }
        ...
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Sensitive fields remain excluded until selected. Global privacy
              mode still masks values.
            </p>
          </div>
      ...
      <article className="estate-print-document mx-auto max-w-5xl bg-white p-6 text-slate-950 shadow-2xl sm:p-10">
        <header className="border-b-2 border-slate-900 pb-6">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Prepared for {content.ownerDisplayName} · Values as of{" "}
            {content.valueAsOfDate} · Generated{" "}
            {new Date(content.generatedAt).toLocaleString()}
          </p>
          {content.plan.jurisdiction ? (
            <p className="mt-1 text-sm text-slate-600">
              Jurisdiction reference: {content.plan.jurisdiction}
            </p>
          ) : null}
        </header>

        <section className="my-6 border border-amber-300 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
          <strong>Planning document only.</strong> {content.disclaimer} It does
          not address witnessing, capacity, probate, guardianship, trusts,
          taxes, or other jurisdiction-specific requirements.
        </section>
        ...
        <section className="print-break-inside-avoid">
          ...
          <div className="mt-3 grid grid-cols-3 border-y border-slate-300 py-4 text-sm">
            <div>
              ...
              <p className="mt-1 font-bold">
                <OptionalMoney
                  amount={content.totals.grossAssetsBaseMinor}
                  currency={content.baseCurrency}
                  visible={includeValues}
                />
              </p>
            </div>
            <div>
              ...
              <p className="mt-1 font-bold">
                <OptionalMoney
                  amount={content.totals.liabilitiesBaseMinor}
                  currency={content.baseCurrency}
                  visible={includeValues}
                />
              </p>
            ...
            <div>
              ...
              <p className="mt-1 font-bold">
                <OptionalMoney
                  amount={content.totals.netEstateBaseMinor}
                  currency={content.baseCurrency}
                  visible={includeValues}
                />
              </p>
          ...
          {!content.totals.complete ? (
            <p className="mt-2 text-sm font-medium text-amber-800">
              Totals are incomplete because one or more exchange rates are
              unavailable.
            </p>
          ) : null}
        </section>
          ...
          <div className="mt-3 space-y-5">
            {content.assets
              ...
              .map((asset) => (
                <section
                  key={asset.id}
                  className="print-break-inside-avoid border-t border-slate-300 pt-4"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h3 className="font-bold">{asset.name}</h3>
                      <p className="mt-1 text-sm text-slate-600">
                        {asset.categoryName} · Ownership share{" "}
                        {(asset.ownershipShareBps / 100).toFixed(2)}% ·{" "}
                        {asset.transferContext.replaceAll("_", " ")} ·{" "}
                        {asset.distributionMethod.replaceAll("_", " ")}
                      </p>
                      {includeReferences && asset.accountReference ? (
                        <p className="mt-1 text-sm">
                          Account reference: {asset.accountReference}
                        </p>
                      ) : null}
                      {includeReferences && asset.documentReference ? (
                        <p className="mt-1 text-sm">
                          Document reference: {asset.documentReference}
                        </p>
                      ) : null}
                    </div>
                      ...
                      {asset.allocations.map((allocation) => (
                        ...
                        >
                          ...
                          <td className="py-2 text-right">
                            {(allocation.allocationBps / 100).toFixed(2)}%
                          </td>
                          <td className="py-2 text-right">
                            <OptionalMoney
        ...
        <section className="mt-8 print-break-inside-avoid">
          <h2 className="text-lg font-bold">Residual estate instructions</h2>
          <p className="mt-1 text-sm text-slate-600">
            These percentages apply to otherwise unallocated portions.
            Contingent recipients are alternatives, not additions to primary
            gifts.
          </p>
          <div className="mt-3 divide-y divide-slate-200 border-y border-slate-300">
            {content.residuaryAllocations.length ? (
              content.residuaryAllocations.map((allocation) => (
                <div
                  key={allocation.id}
                  className="grid grid-cols-[minmax(0,1fr)_auto_auto] gap-4 py-3 text-sm"
                >
                  <strong>{allocation.beneficiaryName}</strong>
              ...
              <p className="py-3 text-sm text-slate-500">
                No residual allocation is recorded.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="mt-8 print-break-inside-avoid">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Indicative totals include primary account gifts and primary residual
            shares only. Contingent instructions are excluded from these totals.
          </p>
          <div className="mt-3 divide-y divide-slate-200 border-y border-slate-300">
            {content.beneficiaries.map((beneficiary) => {
              const total = content.beneficiaryTotals.find(
                (row) => row.beneficiaryId === beneficiary.id,
              );
              return (
                <div
                  key={beneficiary.id}
                ...
                >
                  <div>
                    ...
                    <p className="capitalize text-slate-600">
                      {[beneficiary.kind, beneficiary.relationship]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                    {includeContacts && beneficiary.contactSummary ? (
                      <p className="mt-1">{beneficiary.contactSummary}</p>
                    ) : null}
                    {includeNotes && beneficiary.notes ? (
                      <p className="mt-1 text-slate-600">{beneficiary.notes}</p>
                    ) : null}
                  </div>
                  <p className="font-bold">
                    <OptionalMoney
                      amount={total?.amountBaseMinor ?? "0"}
                      currency={content.baseCurrency}
                      visible={includeValues}
                    />
                  </p>
        ...
        <section className="mt-8 print-break-inside-avoid">
          ...
          <div className="mt-3 divide-y divide-slate-200 border-y border-slate-300">
            {content.liabilities.length ? (
              content.liabilities.map((liability) => (
                <div
                  key={liability.id}

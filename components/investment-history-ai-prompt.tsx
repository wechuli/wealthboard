...
export function investmentHistoryAiPrompt(
  ...
  return [
    ...
    "",
    "ACCOUNT BOUNDARY",
    `- The account currency is ${accountCurrency}, which supports at most ${accountFractionDigits} decimal place${accountFractionDigits === 1 ? "" : "s"} for cash amounts.`,
    ...
    "5. When the source has no stable ID, derive one from immutable source fields. Never use a random UUID, conversion timestamp, or row number. If two records cannot receive distinct deterministic IDs, return NEEDS CLARIFICATION.",

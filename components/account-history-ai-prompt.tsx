...
export function accountHistoryAiPrompt(
  ...
  return [
    ...
    "6. If the source has no ID, create a deterministic ID from immutable source fields using: derived-<date>-<type>-<amount>-<description-slug>. Never use a random UUID, timestamp of this conversion, or row number. If two transactions would receive the same derived ID and the source provides no stable distinguishing detail, return NEEDS CLARIFICATION.",
    ...
    `9. Format amount as a plain decimal string without currency symbols, grouping commas, or scientific notation. ${currency} supports at most ${fractionDigits} decimal place${fractionDigits === 1 ? "" : "s"}.`,

...
export function TransactionForm({
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      const recentType = localStorage.getItem(
        "wealthboard-recent-transaction-type",
      );
      if (
        recentAccount &&
        accounts.some((account) => account.id === recentAccount)
      ) {
        setValue("accountId", recentAccount);
      }
      if (
        recentType &&
        availableTypes.some(([value]) => value === recentType)
      ) {
    ...
    >
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <FieldError>
            {errors.accountId?.message ||
              serverState.fieldErrors?.accountId?.[0]}
          </FieldError>
        </div>
        {type === "transfer" ? (
          <div>
            ...
            <Select id="toAccountId" {...register("toAccountId")}>
              ...
              {accounts
                .filter(
                  (account) => account.id !== accountId && !account.isLiability,
                )
                .map((account) => (
                  <option key={account.id} value={account.id}>
                    {account.name} · {account.currency}
        ...
        <div>
          ...
          />
          <FieldError>
            {errors.amount?.message || serverState.fieldErrors?.amount?.[0]}
          </FieldError>
        </div>
        {type === "transfer" &&
        source &&
        ...
        <div>
          ...
          <Input
            id="transactionDate"
            type="date"
            defaultValue={initial?.transactionDate ?? today}

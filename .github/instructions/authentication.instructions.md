...
- Discard old singleton credentials and unowned portfolio rows during migration. Do not add a previous-password field, legacy claim, recovery panel, or migrated default user to signup.
...
- Test signup for the first and subsequent users, absence of alternate user-creation paths, case-insensitive username collisions, generic login errors, cookie attributes, expiry, disabled users, per-user invalidation, concurrent users, and destructive singleton migration without a recovery UI.

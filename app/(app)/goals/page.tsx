...
export default async function GoalsPage() {
  ...
  const safeTotal = (value: (goal: (typeof active)[number]) => bigint) =>
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <Summary
          label="Active target"
          value={
            <MoneyValue amount={totalTarget} currency={settings.baseCurrency} />
          }
          icon={<Flag size={17} />}
        />
        <Summary
          label="Amount saved"
          value={
            <MoneyValue amount={totalSaved} currency={settings.baseCurrency} />
          }
        ...
        <Summary
          label="Monthly plan"
          value={
            <MoneyValue amount={monthly} currency={settings.baseCurrency} />
          }
                ...
                <CardHeader>
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {goal.accountName || "Directly tracked goal"}
                    </p>
                  </div>
                  <Badge
                    tone={goal.tracking === "behind" ? "warning" : "positive"}
                  >
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 border-t border-white/[0.06] pt-4 text-xs">
                    <div>
                      <p className="text-slate-500">
                        Required monthly ({goal.assumedAnnualReturnBps / 100}%
                        return)
                      </p>
